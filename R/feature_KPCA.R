...
#'
data_feature_func_KPCA <- function(data_raw,lookback_time){
  library(tidyverse)
  library(kernlab)
  ...
  for(j in 1:length(lookback_time)){
    ...
    for (var in vars_to_pivot) {
      pivot_result <- data[,c('hadm_id',all_of(var))] %>%
        group_by(hadm_id) %>%
        mutate(row_num = row_number()) %>%
        pivot_wider(names_from = row_num, values_from = all_of(var),
                    names_prefix = "var_") %>%
        ungroup()
      ...
      # KPCA was performed, radial basis function was chosen and two principal components were selected
      kpca_result <- kpca(~., data = pivot_results[[var]][,-1], kernel = "rbfdot", kpar = list(sigma = 0.1),features=2)
      #Extraction of KPCA principal components
      dr_result <- as.data.frame(kpca_result@pcv)
      #Add hadm_id variable
      dr_result <- cbind(hadm_id = pivot_results[[var]]$hadm_id,dr_result)
      #Rewriting variable names
      colnames(dr_result)[2:3] <- paste0(all_of(var),"_kpca_",1:2)
      #Add to list
    ...
    grouped_measurements <- data %>%
      ...
      summarize(
        ...
        anchor_age = first(anchor_age), #Use first() to get a constant value
        sbp_max = max(sbp_avg),
        sbp_min = min(sbp_avg),
        sbp_var = var(sbp_avg),
        dbp_max = max(dbp_avg),
    ...
    data_1 <- left_join(grouped_measurements, data_kpca, by = "hadm_id")
    # Standardised processing
    data_1 <- data_1 %>%
      mutate(across(3:ncol(data_1), scale))
